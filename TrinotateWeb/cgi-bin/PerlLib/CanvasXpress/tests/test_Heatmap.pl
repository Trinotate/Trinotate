...
print $cgi->start_html(-title => "Testing Heatmap",
                       -onLoad => "load_plots();");


my $heatmap_tester = new CanvasXpress::Heatmap("test_canvas");
my $plot_loader = new CanvasXpress::PlotOnLoader("load_plots");
$plot_loader->add_plot($heatmap_tester);
...
print $heatmap_tester->draw(%inputs);
