...
Ext.canvasXpress = Ext.extend(Ext.Panel, {
  ...
  initComponent: function() {
    ...
    if(!this.listeners.resize)
      this.listeners.resize = function(p, aw, ah, rw, rh) {
        if(this.noResize)
        {
    ...
    switch(this.options.graphType) {
      case 'Network':
        this.addEvents(
          ...
          'loadview','saveview','deleteview','doubleclick');
          ...
          items: [
            {
              ...
              value: cxptest? 'new CanvasXpress( \'canvas\', ' + enc(a[0]) + ', ' + enc(a[1]) + ');' : (type.match(/Import/)? '' : enc(a))
            }
          ]
        }
      }
  ...
  constructor: function(config, extCanvas, callback) {
    ...
    var item2 = [
        ...
        allowBlank: true, name:'imagePath', disabled: config.shape != 'image' },
      {
        ...
        items: [{
          xtype: 'checkbox',
          checked: isMulti? false : config.anchor,
          boxLabel: 'Anchor',
          width: 65,
          name: 'anchor'
        }, {
          ...
          xtype: 'checkbox',
          checked: isMulti? false : config.fixed,
          boxLabel: 'Fixed',
          width: 55,
          name: 'fixed'
        }, {
